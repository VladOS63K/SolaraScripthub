...
function createScriptCard(id, userid, name, description, script) {
    ...
    fetch("http://147.185.221.30:58417/solara-scriptdb/getuserdata?id=" + userid).then(r => {
        if (r.status == 200) {
            r.text().then(text => {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (currentUserId != null) {
        ...
        fetch("http://147.185.221.30:58417/solara-scriptdb/getuserdata?id=" + currentUserId).then(r => {
            if (r.status == 200) {
                r.text().then(text => {
                    var resp = JSON.parse(text);
    ...
    fetch("http://147.185.221.30:58417/solara-scriptdb/getscripts").then(r => {
        r.text().then(text => {
            var resp = JSON.parse(text);
