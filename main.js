...
function createScriptCard(id, userid, name, description, script) {
    ...
    fetch("https://proxy.corsfix.com/?https://147.185.221.19:7132/solara-scriptdb/getuserdata?id=" + userid).then(r => {
        if (r.status == 200) {
            r.text().then(text => {
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (currentUserId != null) {
        ...
        fetch("https://proxy.corsfix.com/?https://147.185.221.19:7132/solara-scriptdb/getuserdata?id=" + currentUserId).then(r => {
            if (r.status == 200) {
                r.text().then(text => {
                    var resp = JSON.parse(text);
    ...
    fetch("https://proxy.corsfix.com/?https://147.185.221.19:7132/solara-scriptdb/getscripts").then(r => {
        r.text().then(text => {
            var resp = JSON.parse(text);
